...
<div class="border_07 bg_02" style="margin-bottom:5px;" id="item_1">
    ...
    <div id="cnt1" style="display:none">
        ...
        <div style="padding:5px 10px 0 10px">
            ...
            ({/if})
            <a href="({t_url m=pc a=page_fh_diary})&amp;target_c_diary_id=({$item.c_diary_id})&amp;c_diary_comment_count=({$item.comment_count})">({$item.subject|t_body:'title'|default:"&nbsp;"})</a>({if $item.public_flag == 'friend'})<img src="({t_img_url_skin filename=friend_icon f=gif})" align="absmiddle">({/if})&nbsp;(コメント({$item.comment_count|default:0})&nbsp;閲覧:({$item.etsuran_count}))(({$item.nickname|t_body:'name'|default:"&nbsp;"}))({if $item.image_filename_1 || $item.image_filename_2 || $item.image_filename_3})<img src="({t_img_url_skin filename=icon_camera})" class="icon">({/if})
        </div>
        ({/foreach})
