...
function db_diary_get_c_diary_comment_list4c_diary_id($c_diary_id)
{
    $sql = 'SELECT cm.nickname, cm.image_filename, cd.*' .
        ' FROM ' . MYNETS_PREFIX_NAME . 'c_diary_comment as cd LEFT JOIN ' . MYNETS_PREFIX_NAME . 'c_member as cm USING (c_member_id)' .
        ' WHERE cd.c_diary_id = ?' .
...
{
    $sql = 'SELECT c_diary_id FROM ' . MYNETS_PREFIX_NAME . 'c_diary WHERE c_member_id = ? AND is_checked = 0 ORDER BY r_datetime';
    $params = array(intval($c_member_id));
    return db_get_one($sql, $params);
}
...
function p_h_home_c_diary_friend_list4c_member_id($c_member_id, $limit)
{
    if (DIARY_DISTINCT_LIST !== true) {
        ...
        foreach ($c_diary_friend_list as $key => $value) {
            ...
            //$c_diary_friend_list[$key]['count_comments'] = $value['comment_count'];
        }
    //コメントカウントの名前が又違う！
        return $c_diary_friend_list;
    }
...
function p_h_home_c_diary_my_comment_list4c_member_id($c_member_id, $limit)
{
    ...
    $sql = 'SELECT dc.c_diary_id' .
            ' FROM ' . MYNETS_PREFIX_NAME . 'c_diary_comment as dc INNER JOIN ' . MYNETS_PREFIX_NAME . 'c_diary as cd USING (c_diary_id)' .
 ...
 */
function p_fh_diary_list_diary_list_date4c_member_id($c_member_id, $page, $page_size, $year, $month, $day=0, $u = null)
{
    if ($day) {
...
{
    $sql = "SELECT cdc.c_diary_comment_id FROM " . MYNETS_PREFIX_NAME . "c_diary as cd, " . MYNETS_PREFIX_NAME . "c_diary_comment as cdc" .
...
function k_p_fh_diary_c_diary_comment_list4c_diary_id($c_diary_id, $page_size, $page, $page_sort = '')
{
    ...
    foreach ($c_diary_comment_list as $key => $value) {
        ...
        $c_diary_comment_list[$key]['body'] = preg_replace_callback('/>>(\d+)/i','make_diary_comment_link',$value['body']);
    }
