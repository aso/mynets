...
<div id="data-list">
    <table>
        ...
        <tr>
            <td class="idnumber"><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_information_data')})&amp;target_c_inquiry_id=({$item.c_admin_information_id})">({$item.c_admin_information_id})</a></td>
            <td><a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('view_information_data')})&amp;target_c_admin_information_id=({$item.c_admin_information_id})">({$item.r_datetime})</a></td>
            <td>({$item.category})</td>
            <td>({$item.subject})</td>
            <td style="width:300px">({$item.body})</td>
            <td>({if $item.c_view_flag == '0'})<a href="?m=({$module_name})&amp;a=do_({$hash_tbl->hash('update_information')})&amp;target_c_information_id=({$item.c_admin_information_id})&amp;flg=9&amp;sessid=({$PHPSESSID})">表示中</a>({else})<a href="?m=({$module_name})&amp;a=do_({$hash_tbl->hash('update_information')})&amp;target_c_information_id=({$item.c_admin_information_id})&amp;flg=0&amp;sessid=({$PHPSESSID})">非表示</a>({/if})</td>
            <td><a href="?m=({$module_name})&amp;a=do_({$hash_tbl->hash('delete_information')})&amp;target_c_information_id=({$item.c_admin_information_id})&amp;sessid=({$PHPSESSID})">削除する</a></td>
        </tr>
        ({/foreach})
        </tbody>
    </table>
