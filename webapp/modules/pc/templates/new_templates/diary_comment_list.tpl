...
<div class="border_07 bg_02" style="margin-bottom:5px;" id="item_3">
    ...
    <div id="cnt3" style="display:none">
        ...
        <div style="padding:5px 10px 0 10px">
            ...
            ({/if})
            <a href="({t_url m=pc a=page_fh_diary})&amp;target_c_diary_id=({$item.c_diary_id})&amp;c_diary_comment_count=({$item.comment_count})">({$item.subject|t_body:'title'})</a>({if $item.public_flag == 'friend'})<img src="({t_img_url_skin filename=friend_icon f=gif})" align="absmiddle">({/if})&nbsp;(コメント:({$item.comment_count})&nbsp;閲覧:({$item.etsuran_count}))&nbsp;(({$item.nickname|t_body:'name'}))
        </div>
        ({/foreach})
