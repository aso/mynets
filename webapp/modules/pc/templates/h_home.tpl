...
<script type="text/javascript">
    var oneword_url = "./?m=pc&a=do_oneword_edit&sessid=({$PHPSESSID})";
</script>
...
({********************************})
                ...
                ({ext_include file="new_templates/myschedule.tpl"})
                ...
                ({**伝言板**})
                ({ext_include file="new_templates/dengon_list.tpl"})

                <div id="rightpane" style="width:440px;">

                ({**フレンド最新日記**})
                <div id="dv1" style="display:none;">({if $c_diary_friend_list})({ext_include file="new_templates/friend_diary_list.tpl"})({/if})</div>
                ({**フレンド最新ブログ**})
                <div id="dv2" style="display:none;">({if $c_rss_cache_list})({ext_include file="new_templates/friend_blog_list.tpl"})({/if})</div>
                ({**日記コメント履歴**})
                <div id="dv3" style="display:none;">({if $c_diary_my_comment_list})({ext_include file="new_templates/diary_comment_list.tpl"})({/if})</div>
                ({**参加コミュニティ最新書き込み**})
                <div id="dv4" style="display:none;">({if $c_commu_topic_comment_list})({ext_include file="new_templates/my_commu_new_list.tpl"})({/if})</div>
                ({**フレンド最新レビュー**})
                <div id="dv5" style="display:none;">({if $c_friend_review_list})({ext_include file="new_templates/friend_new_review_list.tpl"})({/if})</div>
                ({**お気に入り最新日記**})
                <div id="dv6" style="display:none;">({if $bookmark_diary_list})({ext_include file="new_templates/bookmark_diary_list.tpl"})({/if})</div>
                ({**お気に入り最新ブログ**})
                <div id="dv7" style="display:none;">({if $bookmark_blog_list})({ext_include file="new_templates/bookmark_blog_list.tpl"})({/if})</div>
                ({**自己情報一覧**})
                <div id="dv8" style="display:none;">({if $c_diary_list || $c_blog_list || $c_review_list})({ext_include file="new_templates/my_diary_etc_list.tpl"})({/if})</div>
                </div>
                ...
                ({ext_include file="new_templates/myintro.tpl"})
