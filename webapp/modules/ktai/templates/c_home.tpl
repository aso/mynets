...
[({$item.r_datetime|date_format:"%m/%d"})]<a href="({t_url m=ktai a=page_c_bbs})&amp;target_c_commu_topic_id=({$item.c_commu_topic_id})&amp;({$tail})">({$item.name|t_body:'title'})</a>(({$item.count_comments|t_body:'title'}))<br>
({foreachelse})
トピックはありません<br>
({/foreach})
...
[({$item.r_datetime|date_format:"%m/%d"})]<a href="({t_url m=ktai a=page_c_bbs})&amp;target_c_commu_topic_id=({$item.c_commu_topic_id})&amp;({$tail})">({$item.name|t_body:'title'})</a>(({$item.count_comments}))<br>
({foreachelse})
イベントはありません<br>
({/foreach})
